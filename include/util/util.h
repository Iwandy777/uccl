...
#define UCCL_LOG_ERROR UCCL_LOG(ERROR) << "[Error] "
...
static inline jring_t* create_ring(size_t element_size, size_t element_count) {
  ...
  if (jring_init(ring, element_count, element_size, 1, 1) < 0) {
    UCCL_LOG(ERROR) << "Failed to initialize ring buffer";
    free(ring);
    exit(EXIT_FAILURE);
  }
  return ring;
...
static inline bool str_to_mac(std::string const& macStr, char mac[6]) {
  if (macStr.length() != 17) {
    UCCL_LOG(ERROR) << "Invalid MAC address format.";
    return false;
  }
...
inline int get_dev_numa_node(char const* dev_name) {
  ...
  if (numa_node == -1) {
    UCCL_LOG(WARN) << "NUMA node is -1 for " << dev_name
...
inline int get_gpu_numa_node(int device_id) {
  char bdf[64];
...
static inline void pin_thread_to_cpu(int cpu) {
  ...
  if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset)) {
    UCCL_LOG(ERROR) << "Failed to set thread affinity to CPU " << cpu;
  }
}
...
inline void pin_thread_to_numa(int numa_node) {
  ...
  if (!cpumap_file.is_open()) {
    UCCL_LOG(ERROR) << "Failed to open " << cpumap_path;
    return;
  }
  ...
  if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset)) {
    UCCL_LOG(ERROR) << "Failed to set thread affinity to NUMA node "
                    << numa_node;
  }
}
